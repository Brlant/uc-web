        ...
        this.$refs[formName].validate((valid) => {
          ...
          if (this.action === 'add') {
            OrgUser.save(formData).then(() => {
              ...
              this.$notify.error({
                duration: 2000,
                message: e.response && e.response.data && e.response.data.msg || '新增用户"' + self.form.name + '"失败'
              });
              this.doing = false;
            });
          } else {
            OrgUser.update(self.form.id, formData).then(() => {
              ...
              this.$notify.error({
                duration: 2000,
                message: e.response && e.response.data && e.response.data.msg || '修改货主用户"' + self.form.name + '"失败'
              });
              this.doing = false;
            });
          }
