    ...
    <div>
      ...
      <el-table style="width: 100%" header-row-class-name="table-header-color"
        ...
        <el-table-column prop="name" label="操作" min-width="200">
          <template slot-scope="scope">
            <des-btn perm="mdm-manufacture-oms-org-user-edit" icon="edit" @click="editOrgAccount(scope.row)">编辑
            </des-btn>
      ...
      <div class="text-center" v-show="dataRows.length || pager.currentPage !== 1">
  ...
  methods: {
    ...
    getOrgAccountList: function (pageNo) {
      ...
      OrgUser.queryAllTag(data).then(res => {
        ...
        this.pager.count = this.pager.currentPage * this.pager.pageSize + (this.dataRows.length === this.pager.pageSize ? 1 : 0);
      });
    },
